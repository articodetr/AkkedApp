...
import { getCustomerNotificationAttentionCount } from '../services/notificationService';
...
export default function CustomerDetailsScreen() {
  ...
  const loadUnreadNotifications = useCallback(async () => {
    ...
    try {
      const count = await getCustomerNotificationAttentionCount(currentUser.userId, String(id));
      setUnreadNotificationsCount(count);
    } catch (error) {
      console.error("Error loading customer notifications count:", error);
      setUnreadNotificationsCount(0);
    }
    ...
    try {
      ...
      if (notification?.id) {
        router.push({
          ...
          params: { id: notification.id, from: 'customer-details', customerId: String(id), customerName: customer?.name || '', returnToCustomerId: String(id) },
        });
        return true;
      }
  ...
  const customerNotificationsCount = unreadNotificationsCount;
