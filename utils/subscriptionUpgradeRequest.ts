...
// الحد المجاني لعدد العملاء لكل مستخدم (عاديون + مربوطون، باستثناء حساب الأرباح/الخسائر).
// يُستخدم في فحص الواجهة وفي رسائل الاشتراك، ويطابق الحد المفروض في قاعدة البيانات.
...
const buildUpgradeMessage = (
  ...
  const lines = [
    ...
    `لقد وصلت إلى الحد المجاني (${FREE_CUSTOMER_LIMIT} عميل)، وظهرت لي رسالة تفيد بضرورة الاشتراك في التطبيق.`,
    '',
    'بيانات الحساب:',
